    ...
    async def post(self, request: Request, paramters: dict) -> JSONResponse:
        ...
        if paramters:
            data = [
                ...
                async for match, _ in league.matches(**paramters)
            ]
        else:
            data = await CacheMatches(league.league_id).get()
